export const texts = {
  en: {
    oneMinuteTitle: 'One minute left!',
    oneMinuteBody: 'One minute left to finish the activity',
    oneMinuteDismiss: 'Got it',
    loading: 'Loading...',
    error: 'Failed to load activity',
    welcome: 'Welcome!',
    startButton: 'Start',
    step: 'Step',
    game: 'Game',
    nextStation: 'Next Station',
    finish: 'Finish',
    summary: 'Summary',
    totalScore: 'Total Score',
    points: 'points',
    backToHome: 'Back to Home',
    of: 'of',
    stationHint: 'Hint',
    showStationHint: 'Show Hint',
    hintWarning: 'Please note that using a hint reduces 4 points.',
    hintWarningTime: 'Please note that using a hint adds 4 minutes to your time.',
    solutionHint: 'Show solution',
    solutionHintWarning: 'This will fill in the correct answer and add 4 minutes to your time. Continue?',
    solutionHintConfirm: 'Show solution',
    enteringTextRetryTitle: 'Out of attempts',
    enteringTextRetryMessage: 'No worries — moving on to the next station.',
    enteringTextRetryButton: 'Continue',
    hintCancel: 'Cancel',
    hintConfirm: 'Use Hint',
    hintTitle: 'Hint',
    hintClose: 'Got it',
    popupDismiss: 'OK',
    mediaLoadSlow: 'Still loading — check your connection.',
    mediaRetry: 'Try again',
    continueButton: 'Continue',
    videoWatchToContinue: 'Watch the video to continue',
    videoSkip: 'Skip the video',
    imageZoomHint: 'Enlarge image',
    finishTitle: 'LESSON COMPLETE!',
    finishScore: 'Your Score',
    finishStars: 'Points',
    finishTime: 'Time',
    finishItems: 'Items',
    finishComplete: 'Done!',
    finishStatus: 'Status',
    stayHere: 'Stay Here',
    finishSaveFailed: 'Could not save your results. Tap to retry.',
    finishSavePending: 'Saving your results...',
    viewLeaderboard: 'CONTINUE ADVENTURE',
    exitActivity: 'Exit',
    exitConfirmTitle: 'Are you sure?',
    exitConfirmMessage: 'All data will be lost.',
    exitConfirmOk: 'Yes, exit',
    exitConfirmCancel: 'Cancel',
    autoExitIn: 'Auto exit in',
    seconds: 's',
    leaderboardTitle: 'Leaderboard',
    leaderboardBack: 'Back',
    leaderboardRank: '#',
    leaderboardName: 'Name',
    leaderboardScore: 'Score',
    leaderboardEmpty: 'No scores yet',
    leaderboardLoading: 'Loading...',
    leaderboardGroupsTitle: 'Group Scores',
    leaderboardPlayersTitle: 'Players',
    leaderboardMyGroupTitle: 'My Group',
    tapToStart: 'Tap to start!',
    completed: 'Completed',
    locked: 'Locked',
    guidelinesTitle: 'MISSIONS & GUIDELINES',
    guidelinesSubHeader: 'Your Adventure Awaits',
    guidelinesMissionTitle: 'MISSION',
    guidelineMission1: 'Complete all {count} activities along the path',
    guidelineMission2: 'Earn as many points as you can!',
    guidelinesRulesTitle: 'GUIDELINES',
    guidelineRule1: 'Follow the numbered path sequentially',
    guidelineRule2: 'Complete activities to unlock the next point',
    guidelinesCustomLabel: 'Additional Guidelines',
    startAdventure: 'START NOW!',
    mapYou: 'You',
    mapStart: 'Start walking',
    mapOpenStation: 'Open station',
    mapOpenAnyway: 'Open station',
    mapKeepWalking: 'Keep walking to the station',
    mapImHere: "I'm at the station",
    mapLocating: 'Finding your location...',
    mapMeters: 'm away',
    mapAccuracy: 'accuracy',
    mapGeoDenied: 'Location is blocked. Allow it in your browser settings, or tap “I’m at the station” when you get there.',
    mapGeoUnavailable: 'Cannot get your location right now.',
    mapUnavailable: 'The map could not be loaded.',
    mapNoStationLocation: 'This station has no location set.',
  },
  he: {
    oneMinuteTitle: 'נשארה דקה אחת!',
    oneMinuteBody: 'נשארה דקה אחת לסיום הפעילות',
    oneMinuteDismiss: 'הבנתי',
    loading: 'טוען...',
    error: 'שגיאה בטעינת הפעילות',
    welcome: 'ברוכים הבאים!',
    startButton: 'התחילו',
    step: 'שלב',
    game: 'משחק',
    nextStation: 'תחנה הבאה',
    finish: 'סיום',
    summary: 'סיכום',
    totalScore: 'ניקוד כולל',
    points: 'נקודות',
    backToHome: 'חזרה לדף הבית',
    of: 'מתוך',
    stationHint: 'רמז',
    showStationHint: 'הצג רמז',
    hintWarning: 'שימו לב ששימוש ברמז מפחית 4 נקודות.',
    hintWarningTime: 'שימו לב ששימוש ברמז מוסיף 4 דקות לזמן שלך.',
    solutionHint: 'הצג את הפתרון',
    solutionHintWarning: 'הצגת הפתרון תמלא את התשובה הנכונה ותוסיף 4 דקות לזמן שלך. להמשיך?',
    solutionHintConfirm: 'הצג פתרון',
    enteringTextRetryTitle: 'נגמרו הניסיונות',
    enteringTextRetryMessage: 'לא נורא — ממשיכים לתחנה הבאה.',
    enteringTextRetryButton: 'המשך',
    hintCancel: 'ביטול',
    hintConfirm: 'השתמש ברמז',
    hintTitle: 'רמז',
    hintClose: 'הבנתי',
    popupDismiss: 'אישור',
    mediaLoadSlow: 'עדיין טוען — בדקו את החיבור לאינטרנט.',
    mediaRetry: 'נסו שוב',
    continueButton: 'המשך',
    videoWatchToContinue: 'צפו בסרטון עד הסוף כדי להמשיך',
    videoSkip: 'דלגו על הסרטון',
    imageZoomHint: 'להגדלת התמונה',
    finishTitle: 'השלמת את המסלול!',
    finishScore: 'הניקוד שלך',
    finishStars: 'נקודות',
    finishTime: 'זמן',
    finishItems: 'פריטים',
    finishComplete: 'סיום!',
    finishStatus: 'סטטוס',
    stayHere: 'הישארו כאן',
    finishSaveFailed: 'לא הצלחנו לשמור את התוצאות. לחצו לניסיון נוסף.',
    finishSavePending: 'שומרים את התוצאות...',
    viewLeaderboard: 'טבלת מובילים',
    exitActivity: 'יציאה',
    exitConfirmTitle: 'האם אתה בטוח?',
    exitConfirmMessage: 'כל הנתונים יאבדו.',
    exitConfirmOk: 'כן, צא',
    exitConfirmCancel: 'ביטול',
    autoExitIn: 'יציאה אוטומטית בעוד',
    seconds: 'שנ׳',
    leaderboardTitle: 'טבלת מובילים',
    leaderboardBack: 'חזרה',
    leaderboardRank: '#',
    leaderboardName: 'שם',
    leaderboardScore: 'ניקוד',
    leaderboardEmpty: 'אין ניקוד עדיין',
    leaderboardLoading: 'טוען...',
    leaderboardGroupsTitle: 'ניקוד קבוצות',
    leaderboardPlayersTitle: 'שחקנים',
    leaderboardMyGroupTitle: 'הקבוצה שלי',
    tapToStart: 'לחצו להתחיל!',
    completed: 'הושלם',
    locked: 'נעול',
    guidelinesTitle: 'משימות והנחיות',
    guidelinesSubHeader: 'ההרפתקה מחכה לכם',
    guidelinesMissionTitle: 'משימה',
    guidelineMission1: 'יש להשלים את כל הפעילויות לאורך המסלול',
    guidelineMission2: 'צברו כמה שיותר נקודות!',
    guidelinesRulesTitle: 'הנחיות',
    guidelineRule1: 'עקבו אחרי המסלול הממוספר לפי הסדר',
    guidelineRule2: 'השלימו כל פעילות בתחנה כדי לפתוח את התחנה הבאה',
    guidelinesCustomLabel: 'הנחיות נוספות',
    startAdventure: 'התחילו עכשיו!',
    mapYou: 'אתם',
    mapStart: 'יוצאים לדרך',
    mapOpenStation: 'פתחו את התחנה',
    mapOpenAnyway: 'פתחו את התחנה',
    mapKeepWalking: 'המשיכו ללכת אל התחנה',
    mapImHere: 'הגעתי לתחנה',
    mapLocating: 'מאתרים את המיקום שלכם...',
    mapMeters: 'מ׳ מכאן',
    mapAccuracy: 'דיוק',
    mapGeoDenied: 'הגישה למיקום חסומה. אפשרו אותה בהגדרות הדפדפן, או לחצו על ״הגעתי לתחנה״ כשתגיעו.',
    mapGeoUnavailable: 'לא מצליחים לאתר את המיקום כרגע.',
    mapUnavailable: 'לא הצלחנו לטעון את המפה.',
    mapNoStationLocation: 'לא הוגדר מיקום לתחנה הזו.',
  },
};
