...
function withCustomerActivityScope(
  req: Request,
...
router.get("/overview", async (req: Request, res: Response) => {
  ...
  const reportTodayMatch = await reportMatchForRequest(req, {
    joinedAt: { $gte: todayStart },
  });
  const reportWeekMatch = await reportMatchForRequest(req, {
    joinedAt: { $gte: weekStart },
  });
  const reportWithScoreMatch = await reportMatchForRequest(req, {
  ...
  const [
    ...
    Report.find(reportWithScoreMatch, {
      "data.totalScore": 1,
      "data.itemResults.maxPossibleScore": 1,
      completionStatus: 1,
      sessionDurationMs: 1,
    }).lean(),
  ]);
  ...
  const scores = allReports
    .map((r) => {
      ...
      if (raw <= 0) return 0;
      const ceiling = maxScoreForReport(
        r as { data?: { itemResults?: { maxPossibleScore?: number }[] } },
      );
...
router.get("/overview/timeline", async (req: Request, res: Response) => {
  ...
  res.json({
    timeline: pipeline.map((d) => ({ date: d._id, count: d.count })),
  });
});
...
router.get(
  "/activities/:id/items/:index/questions",
  async (req: Request<{ id: string; index: string }>, res: Response) => {
    const activityId = req.params.id;
...
router.get(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const exportType = ((req.query.type as string) ||
      "participants") as AnalyticsExportType;

    const validExportTypes: AnalyticsExportType[] = [
      "executive",
      "participants",
      "scores",
      "progress",
    ...
    if (!validExportTypes.includes(exportType)) {
      res
        ...
        .json({
          error:
            "Invalid export type. Must be one of: executive, participants, scores, progress",
        });
      return;
    }
    ...
    const reportFilter = await reportMatchForRequest(
      req,
      activityReportMatch(activityId, req),
    );
    if (activity.excludedReportIds && activity.excludedReportIds.length > 0) {
      reportFilter._id = {
        $nin: activity.excludedReportIds.map((id) => new Types.ObjectId(id)),
      };
    }
    ...
    await sendWorkbook(
      ...
      `${exportType}_${analyticsPeriod(req)}`,
    );
  },
);
...
async function loadCombinedActivities(
  req: Request,
  res: Response,
): Promise<CombinedReportActivity[] | null> {
  ...
  if (ids.length === 0) {
    res
      .status(400)
      .json({ error: "Provide at least one valid activity id in ?ids=" });
    return null;
  }
  if (ids.length > MAX_COMBINED_ACTIVITIES) {
    res
      .status(400)
      .json({ error: `Too many activities (max ${MAX_COMBINED_ACTIVITIES})` });
    return null;
  }
  ...
  const ownedById = new Map(
    activities
      .filter((a) => customerOwnsDoc(req, a))
  ...
  return ids
    ...
    .filter((a): a is NonNullable<typeof a> =>
      Boolean(a),
    ) as unknown as CombinedReportActivity[];
}
...
router.get("/combined/export", async (req: Request, res: Response) => {
  ...
  const exportType = ((req.query.type as string) ||
    "executive") as AnalyticsExportType;
  const validExportTypes: AnalyticsExportType[] = [
    "executive",
    "participants",
    "scores",
  ...
  const perActivity: CombinedExportActivity[] = await Promise.all(
    activities.map(async (activity) => ({
      ...
      reports: (await getExportReports(
        String(activity._id),
        period,
        activity.excludedReportIds,
  ...
  const buffer = await buildCombinedActivitiesWorkbook(
    perActivity,
    exportType,
    reportCard,
  );
...
router.get(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const excluded = new Set(
      (activity.excludedReportIds ?? []).map((id) => String(id)),
    );
    ...
    res.json({
      participants: reports.map((r) => ({
        ...
        score: normalizeScore(
          (r.data as { totalScore?: number })?.totalScore ?? 0,
          ceiling,
        ),
...
router.patch(
  "/activities/:id/participants/exclusions",
  async (req: Request<{ id: string }>, res: Response) => {
    const activityId = req.params.id;
    if (!Types.ObjectId.isValid(activityId)) {
      res.status(400).json({ error: "Invalid activity ID" });
    ...
    const incoming: unknown[] = Array.isArray(req.body?.excludedReportIds)
      ? req.body.excludedReportIds
      : [];
    const ids = [
      ...new Set(
        incoming.filter(
...
router.post(
  "/activities/:id/participants/:reportId/reset-progress",
  async (req: Request<{ id: string; reportId: string }>, res: Response) => {
    const activityId = req.params.id;
    const reportId = req.params.reportId;
    if (
      !Types.ObjectId.isValid(activityId) ||
      !Types.ObjectId.isValid(reportId)
    ) {
      res.status(400).json({ error: "Invalid activity or report ID" });
      return;
    }
    ...
    report.completionStatus = "joined";
    report.lastActiveItemIndex = 0;
    report.totalItemsCompleted = 0;
    report.sessionStartedAt = undefined;
    report.sessionCompletedAt = undefined;
...
router.patch(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    const passThreshold =
      req.body?.passThreshold === null
        ? null
        : clampPassThreshold(req.body?.passThreshold);
