...
// GET /api/admin/mission-folders — list folders (customer-scoped, sorted by name).
router.get('/', authenticateAdmin, async (req: Request, res: Response) => {
  const folders = await MissionFolder.find(customerMongoFilter(req)).sort({ name: 1 }).lean();
  res.json({ folders });
});

// POST /api/admin/mission-folders — create a folder
...
// PATCH /api/admin/mission-folders/:id — rename / recolor a folder
...
// DELETE /api/admin/mission-folders/:id — delete a folder; member missions return to ungrouped.
